...
# ───── PostgreSQL (psycopg sync + psycopg async) ───────────────────────
...
# ───── aiomysql (async) ────────────────────────────────────────────────
# aiomysql wraps pymysql, which shares mysql-connector's errno space
# for the relevant network errors. Kept as a separate name so future
# aiomysql-only patterns have a clear home.
AIOMYSQL_TRANSIENT_CONNECT_ERRNOS: FrozenSet[int] = MYSQL_TRANSIENT_CONNECT_ERRNOS
AIOMYSQL_TRANSIENT_CONNECT_SQLSTATE_PREFIXES: Tuple[str, ...] = MYSQL_TRANSIENT_CONNECT_SQLSTATE_PREFIXES
AIOMYSQL_TRANSIENT_CONNECT_MESSAGE_PREFIXES: Tuple[str, ...] = ()

# ───── Combined views (driver-agnostic retry consumers use these) ─────
_ALL_SQLSTATES: FrozenSet[str] = PG_TRANSIENT_CONNECT_SQLSTATES
_ALL_ERRNOS: FrozenSet[int] = (
    MYSQL_TRANSIENT_CONNECT_ERRNOS | AIOMYSQL_TRANSIENT_CONNECT_ERRNOS
)
...
_ALL_MESSAGE_PREFIXES: Tuple[str, ...] = (
    ...
    + AIOMYSQL_TRANSIENT_CONNECT_MESSAGE_PREFIXES
)
