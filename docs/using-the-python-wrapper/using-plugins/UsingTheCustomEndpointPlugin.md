...
### Async wrapper behavior difference

Under the async wrapper (`aws_advanced_python_wrapper.aio`), `wait_for_custom_endpoint_info_timeout_ms` has a **softer failure mode** than the sync wrapper: when the wait times out, the connection is returned anyway rather than raising an exception. The first query on that connection may see empty member-instance-ids and bypass custom-endpoint filtering until the next monitor refresh populates the cache. This trade-off keeps async callers from getting blocked on monitor initialization when a valid-but-unfiltered connection is acceptable.

If strict "no-connection-until-monitor-ready" behavior is required, set `wait_for_custom_endpoint_info=true` (default) and size `wait_for_custom_endpoint_info_timeout_ms` generously, then inspect the connection's behavior before issuing queries. Alternatively, call the monitor's `wait_for_info(timeout_sec)` directly from the async caller if finer control is needed.
