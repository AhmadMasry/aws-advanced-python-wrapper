...
The AWS Advanced Python Wrapper currently uses [SqlAlchemy](https://docs.sqlalchemy.org/en/20/core/pooling.html) to create and maintain its internal connection pools. The [Postgres](../../examples/PGReadWriteSplitting.py) or [MySQL](../../examples/MySQLReadWriteSplitting.py) sample code provides a useful example of how to enable this feature. For consumers already using SQLAlchemy in their application, see the SA-flavored examples: [`PGSQLAlchemyReadWriteSplitting.py`](../../examples/PGSQLAlchemyReadWriteSplitting.py) and [`MySQLSQLAlchemyReadWriteSplitting.py`](../../examples/MySQLSQLAlchemyReadWriteSplitting.py). The steps are as follows:
